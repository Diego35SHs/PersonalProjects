...
if (nombreUsuSueno($codigoUsuario, $link) == null) {
...
<script>
    ...
    $(document).on("click", ".resetDesc", function() {
        ...
        var button = $(this);
        var paquete = "function=resetDescUsu&id_usu=" + id_usu;
        $.ajax({
            type: "POST",
            ...
            alert("Se restableció la descripción.");
            location.reload();
        }).fail(function(respuesta) {
            alert("No se pudo restablecer la descripción del usuario debido a un posible error de conexión");
        })
    });
